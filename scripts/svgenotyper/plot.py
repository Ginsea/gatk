...
class GATKSVPlottingSuite(object):
    def __init__(self,
        ...
        if args.non_depth_only:
            ...
            mt = mt.filter_rows(mt.info.ALGORITHMS != 'depth')
        if args.min_median_gq is not None:
            mt = mt.annotate_rows(median_gq=hl.agg.approx_median(mt.GQ))
            mt = mt.filter_rows(mt.median_gq >= args.min_median_gq).drop('median_gq')
    ...
    def plot_contig_variant_position_histograms(self):
        ...
        def _histograms_plotter(out_path: str, contig: str, positions_contig: list, types=list):
            ...
            for i in range(len(types)):
                axes[i].set_title("{} Breakend Distribution - {}".format(types[i], contig))
                axes[i].hist(positions_contig[i], bins=n_bins)
    ...
    def plot_variant_counts(self):
        ...
        def _totals_by_contig_plotter(out_path: str, totals_by_contig: list, types: list):
            ...
            for i in range(len(types)):
                axes[i].set_title("Variant Counts - {}".format(types[i]))
                axes[i].bar(x=self.contigs, height=[totals_by_contig[(c, self.types[i])] if (c, self.types[i]) in totals_by_contig else 0 for c in self.contigs])
                axes[i].set_ylabel('freq')
    ...
    def plot_info_field_histograms(self):
        ...
        for i in range(len(self.types)):
            for j in range(len(INFOS_LIST)):
                ...
                axes[i, j].hist(x, bins=bins[INFOS_LIST[j]], density=True)
                axes[i, j].set_yticks([])
                if j == 0:
                    axes[i, j].set_ylabel(self.types[i])
                if i == 0:
    ...
    def plot_genotype_ternary_diagrams(self):
        ...
        for i in range(len(self.types)):
            type = self.types[i]
            gt_freq = mt.filter_rows(mt.info.SVTYPE == type).gt_fractions.collect()
            _draw_ternary(gt_freq, axis=axes[i], label=type)
        plt.tight_layout()
        plt.savefig(out_path)
...
def parse_args():
    ...
    parser.add_argument('--min-median-gq', help='Plot variants above this median GQ (inclusive)', type=int)
